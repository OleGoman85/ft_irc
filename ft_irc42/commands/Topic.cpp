 ...
 * If a colon (':') is present in the command following the channel name, the command is interpreted
 * as a request to change the topic. Otherwise, it is assumed that the client wants to view the current topic.
 *
 * When changing the topic, if the channel is set to topic-restricted mode, the client must be an operator.
 * The updated topic is then broadcast to all members of the channel.
 * If viewing the topic, the current topic (if set) is sent back to the client.
 *
 ...
 *               Expected tokens are: "TOPIC", <channel>, and optionally a new topic (preceded by a colon).
...
void handleTopicCommand(Server* server, int fd, const std::vector<std::string>& tokens, const std::string& command) {
    ...
    // Check if a new topic is specified in the command (i.e., there is a colon).
    ...
    if (topicPos != std::string::npos) {
        ...
        // If the channel is topic-restricted, verify that the client is an operator.
        ...
        // If no new topic is provided, the client is requesting to view the current topic.
