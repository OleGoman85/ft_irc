...
Channel& getOrCreateChannel(Server* server, const std::string& channelName) {
    auto it = server->_channels.find(channelName);
    if (it == server->_channels.end()) {
        auto res = server->_channels.emplace(channelName, Channel(channelName));
        if (!res.second) {
            std::cerr << "Failed to create channel: " << channelName << std::endl;
...
bool basicChannelChecks(int fd, Channel& channel) {
    if (channel.isInviteOnly() && !channel.isOperator(fd)) {
        sendErrorMessage(fd, "473 " + channel.getName() + " :Cannot join channel (+i)\r\n");
        return false;
    }
    if (channel.getUserLimit() > 0 && (int)channel.getClients().size() >= channel.getUserLimit()) {
        sendErrorMessage(fd, "471 " + channel.getName() + " :Channel is full\r\n");
        return false;
    }
    return true;
}
