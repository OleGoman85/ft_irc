 ...
 */
static void handleFileSend(Server* server,
                           int fd,
                           const std::vector<std::string>& tokens)
{
    // We expect: tokens: [0]="FILE" [1]="SEND" [2]=nickname [3]=filename [4]=filesize
    if (tokens.size() < 5) {
 ...
 */
static void handleFileData(Server* server,
                           int fd,
                           const std::vector<std::string>& tokens)
{
 ...
 */
static void handleFileEnd(Server* server,
                          int fd,
                          const std::vector<std::string>& tokens)
{
    ...
    // Send the file to the receiver (for demonstration).
    {
        ...
        // in an IRC client). One could base64 it again, or do chunked sends.
        if (!fileBuf.empty()) {
