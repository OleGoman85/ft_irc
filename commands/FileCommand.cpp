 ...
 *        No external library is used due to project constraints.
 */
...
/**
 * @brief Handles "FILE SEND <nickname> <filename> <filesize>".
 */
static void handleFileSend(Server* server, int fd,
                           const std::vector<std::string>& tokens)
{
    // We expect: tokens: [0]="FILE" [1]="SEND" [2]=nickname [3]=filename
    // [4]=filesize
    if (tokens.size() < 5)
    {
    ...
    // If a transfer with this key already exists, we can either overwrite or
    // reject
    ...
    // Notify the receiver
    {
        std::string msg = ":" + server->getClients()[fd]->getNickname() +
                          " PRIVMSG " + targetNick +
                          " :Incoming file: " + filename + " (" + filesizeStr +
...
/**
 * @brief Handles "FILE DATA <filename> <base64_chunk>".
 */
static void handleFileData(Server* server, int fd,
                           const std::vector<std::string>& tokens)
{
    // We expect: tokens: [0]="FILE" [1]="DATA" [2]=filename [3]=base64_chunk
    if (tokens.size() < 4)
    {
    ...
    // Optionally notify about progress
    {
        ...
        oss << "NOTICE " << server->getClients()[fd]->getNickname()
            << " :Uploaded " << ft.getReceivedBytes() << "/" << ft.getFilesize()
...
/**
 * @brief Handles "FILE END <filename>".
 */
static void handleFileEnd(Server* server, int fd,
                          const std::vector<std::string>& tokens)
{
    // We expect: tokens: [0]="FILE" [1]="END" [2]=filename
    if (tokens.size() < 3)
    {
    ...
    if (!complete)
    {
        std::string msgSender =
            "NOTICE " + server->getClients()[fd]->getNickname() +
    ...
    // Send the file to the receiver (for demonstration).
    {
        ...
        std::ostringstream oss;
        oss << "NOTICE " << server->getClients()[receiverFd]->getNickname()
        ...
        // Here, we just do a direct send of raw data (which may look like
        // garbage in an IRC client). One could base64 it again, or do chunked
        // sends.
        if (!fileBuf.empty())
        {
...
static std::vector<char> base64Decode(const std::string& encodedString)
{
           ...
           isBase64(encodedString[in_]))
    {
        ...
        if (i == 4)
        {
            ...
            charArray3[0] =
                (char)((charArray4[0] << 2) + ((charArray4[1] & 0x30) >> 4));
            charArray3[1] = (char)(((charArray4[1] & 0x0F) << 4) +
                                   ((charArray4[2] & 0x3C) >> 2));
            charArray3[2] =
                (char)(((charArray4[2] & 0x03) << 6) + charArray4[3]);
